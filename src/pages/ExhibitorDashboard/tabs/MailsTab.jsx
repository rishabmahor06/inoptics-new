    ...
    <SectionShell icon={MdMail} iconBg="#dbeafe" iconColor="#3b82f6" title="Mails" subtitle="Sent mail history">
      <div className="flex rounded-lg border border-zinc-200 overflow-hidden" style={{ minHeight: 400 }}>
          ...
          {MAILS.map(m => (
            <button key={m.id} onClick={() => setSelected(m)}
              className={`w-full text-left px-4 py-3.5 border-b border-zinc-100 transition-colors hover:bg-white
                ${selected?.id === m.id ? 'bg-white border-l-2 border-l-zinc-900' : ''}`}>
              <p className="text-sm font-semibold text-zinc-800 truncate">{m.subject}</p>
              <p className="text-xs text-zinc-400 mt-0.5">{new Date(m.sent_at).toLocaleString('en-IN')}</p>
            </button>
          ))}
        </div>
        {/* Body */}
        <div className="flex-1 p-5 overflow-y-auto bg-white">
          {selected ? (
            <>
              <p className="text-base font-bold text-zinc-900 mb-0.5">{selected.subject}</p>
              <p className="text-xs text-zinc-400 mb-4">{new Date(selected.sent_at).toLocaleString('en-IN')}</p>
              <div className="text-sm text-zinc-700 leading-relaxed" dangerouslySetInnerHTML={{ __html: selected.content }} />
            </>
          ) : (
            <div className="h-full flex items-center justify-center">
              <p className="text-sm text-zinc-300">Select a mail to view</p>
            </div>
          )}
        </div>
      </div>
