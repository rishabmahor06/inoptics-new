...
export function Modal({ title, onClose, wide = false, children }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div
        className={`bg-white rounded shadow-2xl flex flex-col overflow-hidden ${wide ? 'w-full max-w-4xl' : 'w-full max-w-2xl'}`}
...
export function SectionShell({ icon: Icon, iconBg, iconColor, title, subtitle, onAdd, addLabel = 'Add', children }) {
  return (
    <div className="bg-white rounded border border-zinc-200 shadow-sm overflow-hidden h-full flex flex-col">
  ...
  return (
    <div className="flex justify-end gap-2 mt-4 pt-4 border-t border-zinc-100">
      <button onClick={onCancel}
        className="px-4 py-2 text-[12px] font-semibold text-zinc-600 bg-zinc-100 rounded hover:bg-zinc-200 transition-colors">
        Cancel
      </button>
      <button onClick={onSave} disabled={saving}
        className="flex items-center gap-1.5 px-4 py-2 text-[12px] font-semibold text-white bg-blue-600 rounded hover:bg-blue-700 transition-colors disabled:opacity-60">
