...
export default function AddressCard({ addr, onEdit, onDelete, onSetActive }) {
  ...
  return (
    <div className={`relative flex flex-col rounded-xl border p-4 transition-all h-full
      ${isActive ? 'border-blue-400 bg-blue-50/60 shadow-sm' : 'border-zinc-200 bg-white hover:border-zinc-300'}`}>

      {/* Header */}
      <div className="flex items-start justify-between mb-3">
        <div>
          <p className="text-[11px] font-bold uppercase tracking-widest text-zinc-400">{addr.label || 'Address'}</p>
          {isActive && (
        ...
        {!isActive && hasData && (
          <button onClick={() => onSetActive(addr.id)} title="Set as active"
      ...
      {/* Content */}
      {hasData ? (
        <div className="flex-1 space-y-0.5 text-[12px] text-zinc-600">
          <p className="font-medium text-zinc-800">{addr.address}</p>
          {(addr.state || addr.pincode) && <p>{[addr.state, addr.pincode].filter(Boolean).join(' – ')}</p>}
          {addr.phone  && <p className="text-zinc-500">Ph: {addr.phone}</p>}
          {addr.email  && <p className="text-zinc-500 truncate">Email: {addr.email}</p>}
          {addr.gst    && <p className="text-zinc-500">GST: {addr.gst}</p>}
        </div>
      ) : (
        <div className="flex-1 flex items-center justify-center">
          <p className="text-zinc-300 text-[12px] italic">No data — click Edit to fill</p>
        </div>
      )}
      ...
      <div className="flex gap-2 mt-4 pt-3 border-t border-zinc-100">
        ...
        {hasData && (
          <button onClick={() => onDelete(addr.label)}
