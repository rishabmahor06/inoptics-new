...
const EMPTY = { label: '', address: '', state: '', pincode: '', phone: '', email: '', gst: '' };

const FIELDS = [
  { key: 'label',   label: 'Label',   placeholder: 'e.g. Head Office',         required: true },
...
export default function AddressModal({ editing, onClose, onSave }) {
  ...
  const inputCls = 'w-full px-3 py-2.5 text-sm border border-zinc-200 rounded-lg bg-zinc-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder:text-zinc-300';

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-100 shrink-0">
          <p className="text-[14px] font-bold text-zinc-800">{isEditing ? 'Edit' : 'Add'} Company Address</p>
