...
function Modal({ editing, onClose, onSave }) {
  ...
  useEffect(() => { setForm(editing?.id ? { area: editing.area || '' } : EMPTY); }, [editing]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm">
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-100">
          <p className="text-[14px] font-bold text-zinc-800">{editing?.id ? 'Edit' : 'Add'} Stall Area</p>
          <button onClick={onClose} className="p-1 rounded-lg text-zinc-400 hover:bg-zinc-100"><MdClose size={18} /></button>
        </div>
        <div className="p-5">
          <label className="block text-[11px] font-semibold text-zinc-500 uppercase tracking-wide mb-1.5">Stall Area <span className="text-red-500">*</span></label>
          <input value={form.area} onChange={e => setForm({ area: e.target.value })} placeholder="e.g. Hall A"
...
export default function StallAreas() {
  const { rows, loading, fetch, add, update, delete: del } = useStallAreasStore();
  const [search, setSearch] = useState('');
  const [modal, setModal] = useState(null);

  useEffect(() => { fetch(); }, []);

  const filtered = rows.filter(r => r.area?.toLowerCase().includes(search.toLowerCase()));

  const handleDelete = (id) => { if (window.confirm('Delete this stall area?')) del(id); };
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <div>
          <h2 className="text-sm font-semibold text-zinc-800">Stall Areas</h2>
          <p className="text-xs text-zinc-400 mt-0.5">{rows.length} area{rows.length !== 1 ? 's' : ''} configured</p>
        </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={3} className="px-4 py-10 text-center text-sm text-zinc-400">No stall areas found</td></tr>
              ) : (
                filtered.map((row, i) => (
                  <tr key={row.id} className="border-b border-zinc-100 last:border-0 hover:bg-zinc-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-[13px] font-medium text-zinc-800">{row.area}</td>
