...
const VISITOR_GUIDE_PATHS = [
  "/visitor-guide", "/visitor-guide/metro-map", "/visitor-guide/exhibition-map",
  "/visitor-guide/weather", "/visitor-guide/tourist-spots", "/visitor-guide/exhibitor-list",
];
...
export default function Navbar() {
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-40 h-[77px] bg-white shadow-sm ">
        <div className="max-w-[1400px] mx-auto h-full px-4 sm:px-6 lg:px-10 flex items-center justify-between gap-4">
          ...
          <ul className="hidden lg:flex items-center gap-1">
            {NAV_ITEMS.map((item, i) => {
              const active =
                item.type === "link" ? isActive(item.to) : isPathGroup(item.paths);
              ...
              return (
                <li key={i} className="relative group">
                  <Link
                    to={item.to}
                    className={`relative flex items-center gap-1 px-3 py-3 text-[14px] font-medium whitespace-nowrap transition-colors
                      ${active ? "text-blue-600" : "text-zinc-700 hover:text-blue-600"}`}
                  >
                    {item.label}
                    <MdKeyboardArrowDown size={16} className="group-hover:rotate-180 transition-transform" />
                    <span
                      className={`absolute left-3 right-7 bottom-1.5 h-0.5 bg-blue-600 origin-left transition-transform duration-300 ease-out
                        ${active ? "scale-x-100" : "scale-x-0 group-hover:scale-x-100"}`}
                    />
                  </Link>
                  <ul className="absolute left-0 top-full pt-1 min-w-[200px] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                    <div className="bg-white border border-zinc-200 rounded-lg shadow-xl overflow-hidden">
                      {item.children.map((c, j) => (
                        <li key={j}>
                          <Link
                            to={c.to}
                            className="block px-4 py-2.5 text-[13px] text-zinc-700 hover:bg-blue-50 hover:text-blue-600 transition-colors"
                          >
                            {c.label}
                          </Link>
                        </li>
      ...
      >
        ...
        <div
          className={`absolute top-[77px] right-0 w-[85%] max-w-[340px] h-[calc(100vh-77px)] bg-white shadow-xl overflow-y-auto transition-transform
        ...
        >
          <ul className="py-2">
            {NAV_ITEMS.map((item, i) => {
                  ...
                  <li key={i}>
                    <Link
                      ...
                      className={`relative block px-5 py-4 text-[15px] font-medium border-b border-zinc-100 transition-colors
                        ${active ? "text-blue-600 bg-blue-50" : "text-zinc-700 hover:text-blue-600 hover:bg-zinc-50"}`}
                    >
                      {active && (
              ...
              const active = isPathGroup(item.paths);
              return (
                <li key={i} className="border-b border-zinc-100">
                  <button
                    onClick={() => setOpenSub(open ? null : i)}
                    className={`relative w-full flex items-center justify-between px-5 py-4 text-[15px] font-medium transition-colors
                      ${active ? "text-blue-600 bg-blue-50" : "text-zinc-700"}`}
                  >
                    {active && (
                      <span className="absolute left-0 top-2 bottom-2 w-1 rounded-r bg-blue-600" />
                    )}
                      ...
                      {item.children.map((c, j) => (
                        <li key={j}>
                          <Link
                            to={c.to}
                            onClick={() => setIsMenuOpen(false)}
                            className="block px-8 py-2.5 text-[13px] text-zinc-700 hover:text-blue-600"
                          >
                            {c.label}
                          </Link>
                        </li>
